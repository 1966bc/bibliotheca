...
--- who wants to understand how a web application actually works,
in anno Domini 2026. From the database to the browser, with
nothing in between but your own code.
...
\tightlist
\item
  \textbf{PHP}: https://phptherightway.com,
  https://phpdelusions.net
\item
  \textbf{JavaScript, HTML, CSS}: https://developer.mozilla.org
\item
  \textbf{SQLite}: https://sqlite.org/docs.html
\end{itemize}
...
Permissions are not only about Linux file ownership. There is
another kind of permission problem: \textbf{who is allowed to
make changes} through the API.

\hypertarget{the-attack}{%
\subsection{The attack}\label{the-attack}}

Imagine you have the Bibliotheca page open. A malicious website
in another tab could contain hidden JavaScript that sends a POST
request to
\texttt{http://localhost/bibliotheca/public/api/publishers.php}
with \texttt{\{"name":\ "HACKED"\}}. The browser would happily
send it --- from the browser's perspective, it is just another
HTTP request to localhost.

This is called \textbf{CSRF} (Cross-Site Request Forgery): a
malicious site tricks the browser into making requests to
\emph{your} application.
...
\end{Shaded}
\item
  JavaScript reads the token and includes it in every
  POST/PUT/DELETE:
...
\NormalTok{headers}\OperatorTok{:}\NormalTok{ \{}
    \StringTok{\textquotesingle{}Content{-}Type\textquotesingle{}}\OperatorTok{:} \StringTok{\textquotesingle{}application/json\textquotesingle{}}\OperatorTok{,}
    \StringTok{\textquotesingle{}X{-}CSRF{-}Token\textquotesingle{}}\OperatorTok{:} \BuiltInTok{document}\OperatorTok{.}\FunctionTok{querySelector}\NormalTok{(}\StringTok{\textquotesingle{}meta[name="csrf{-}token"]\textquotesingle{}}\NormalTok{)}\OperatorTok{.}\AttributeTok{content}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}
\item
...
API rejects its request with \texttt{403\ Forbidden}.
...
\tightlist
\item
  \texttt{Csrf::start()} --- starts a PHP session (with secure
  cookie settings).
\item
  \texttt{Csrf::token()} --- returns the token, generating one if
  needed.
\item
  \texttt{Csrf::verify()} --- compares the \texttt{X-CSRF-Token}
  header against the session; exits with 403 on mismatch.
\end{itemize}

GET requests do not need CSRF protection --- they only read data,
they never modify it. Only POST, PUT, and DELETE are checked.

\hypertarget{verify-it-works}{%
\subsection{Verify it works}\label{verify-it-works}}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Without token → 403}
\ExtensionTok{curl} \AttributeTok{{-}s} \AttributeTok{{-}o}\NormalTok{ /dev/null }\AttributeTok{{-}w} \StringTok{"\%\{http\_code\}"} \AttributeTok{{-}X}\NormalTok{ POST }\DataTypeTok{\textbackslash{}}
...
\CommentTok{\# Prints: 403}

\CommentTok{\# With token → 200 (get token from page first)}
\VariableTok{TOKEN}\OperatorTok{=}\VariableTok{$(}\ExtensionTok{curl} \AttributeTok{{-}s} \AttributeTok{{-}c}\NormalTok{ /tmp/c.txt http://localhost/bibliotheca/public/ }\DataTypeTok{\textbackslash{}}
    \KeywordTok{|} \FunctionTok{grep}\NormalTok{ csrf{-}token }\KeywordTok{|} \FunctionTok{sed} \StringTok{\textquotesingle{}s/.*content="\textbackslash{}([\^{}"]*\textbackslash{})".*/\textbackslash{}1/\textquotesingle{}}\VariableTok{)}
\ExtensionTok{curl} \AttributeTok{{-}s} \AttributeTok{{-}o}\NormalTok{ /dev/null }\AttributeTok{{-}w} \StringTok{"\%\{http\_code\}"} \AttributeTok{{-}b}\NormalTok{ /tmp/c.txt }\AttributeTok{{-}X}\NormalTok{ POST }\DataTypeTok{\textbackslash{}}
\NormalTok{     http://localhost/bibliotheca/public/api/publishers.php }\DataTypeTok{\textbackslash{}}
     \AttributeTok{{-}H} \StringTok{"Content{-}Type: application/json"} \DataTypeTok{\textbackslash{}}
     \AttributeTok{{-}H} \StringTok{"X{-}CSRF{-}Token: }\VariableTok{$TOKEN}\StringTok{"} \DataTypeTok{\textbackslash{}}
     \AttributeTok{{-}d} \StringTok{\textquotesingle{}\{"name": "Test"\}\textquotesingle{}}
\CommentTok{\# Prints: 200}
\end{Highlighting}
\end{Shaded}
...
\ExtensionTok{curl} \AttributeTok{{-}X}\NormalTok{ POST }\AttributeTok{{-}H} \StringTok{"Content{-}Type: application/json"} \DataTypeTok{\textbackslash{}}
     \AttributeTok{{-}d} \StringTok{\textquotesingle{}\{"name":"Test"\}\textquotesingle{}} \DataTypeTok{\textbackslash{}}
\NormalTok{     http://localhost/bibliotheca/public/api/publishers.php}
\end{Highlighting}
\end{Shaded}
...
\item
  \textbf{Elements} --- the live HTML. Useful to check if
  JavaScript actually built the DOM you expected.
\end{itemize}
...
You can inspect the database directly with the \texttt{sqlite3}
command:
...
\CommentTok{{-}{-} Show the schema of a table (columns, types, constraints)}
...
trail: browser console, network tab, Apache log, PHP command
line. The answer is always there --- you just have to look in the
right place.
...
\item
  \textbf{CSRF} --- session-based tokens on every state-changing
  request (Chapter 06).
\item
...
These are non-negotiable. This chapter covers the next layer:
what happens \emph{around} your code.

\hypertarget{session-fixation}{%
\section{Session fixation}\label{session-fixation}}

A session fixation attack works like this: the attacker gives you
a session ID they already know (via a crafted URL or a cookie),
then waits for you to log in. Now they share your authenticated
session.
...
\item
  ...
  language.
\item
  \textbf{Data flows in one direction.} Browser → API → Model →
  Database, and back. Each layer has one job. If you respect the
  boundaries, the code stays simple.
\item
  \textbf{Security is not a feature.} It is a habit. Prepared
  statements, \texttt{textContent}, server-side validation ---
  these are not optional extras. They are how you write code.
\item
  \textbf{The database is the truth.} Everything else --- the
  API, the frontend, the cache --- is a representation. When in
  doubt, ask the database.
\end{itemize}
...
\tightlist
\item
  \textbf{A router} --- you built one in \texttt{index.php}.
  Thirty lines. Laravel's router does the same thing with more
  features.
\item
  \textbf{An ORM} --- you built models with raw SQL. Eloquent
  writes the SQL for you. But when it generates a slow query, you
  will know why --- because you understand \texttt{JOIN} and
  \texttt{INDEX}.
\item
  \textbf{A template engine} --- you used \texttt{textContent} in
  JavaScript. React does the same with JSX, plus reactivity. But
  the DOM manipulation underneath is identical.
\item
  \textbf{Middleware} --- your API endpoints check the HTTP
  method manually. Express or Laravel route middleware does this
  declaratively.
\end{itemize}

A framework is not magic. It is someone else's
\texttt{index.php}, someone else's \texttt{DBMS.php}, someone
else's \texttt{BookView.js} --- packaged, tested, and documented.
You can use it with confidence now, because you know what it
replaces.
...
decide whether the endpoint should exist, what it should
validate, or how it fits into the architecture. Those decisions
require understanding --- the kind you build by writing code from
scratch, breaking it, and fixing it.
