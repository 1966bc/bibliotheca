...
Version 1.618 --- Second edition, MMXXVI AD / MMDCCLXXIX \textit{ab urbe condita}
...
\hypertarget{unsupported-methods}{%
\section{Unsupported methods}\label{unsupported-methods}}

Our controllers dispatch on four methods: GET, POST, PUT, DELETE.
What happens if someone sends a PATCH? The \texttt{else} branch
returns HTTP 405 (Method Not Allowed) --- but not alone. RFC 7231
asks the server to include an \texttt{Allow} header listing the
methods it does accept:

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{http\_response\_code}\NormalTok{(}\DecValTok{405}\NormalTok{)}\OtherTok{;}
\FunctionTok{header}\NormalTok{(}\StringTok{\textquotesingle{}Allow: GET, POST, PUT, DELETE\textquotesingle{}}\NormalTok{)}\OtherTok{;}
\KeywordTok{echo} \FunctionTok{json\_encode}\NormalTok{([}\StringTok{\textquotesingle{}error\textquotesingle{}}\NormalTok{ =\textgreater{} }\StringTok{\textquotesingle{}Method not allowed\textquotesingle{}}\NormalTok{])}\OtherTok{;}
\end{Highlighting}
\end{Shaded}
...
\item
  ...
  disabled.
\item
  \textbf{Missing records} --- before updating or deleting, the
  controller checks that the record exists via
  \texttt{getById()}. If it does not, the API returns HTTP 404
  (Not Found). Without this check, an \texttt{UPDATE} or
  \texttt{DELETE} on a non-existent ID would affect zero rows and
  the API would still answer 200: a silent bug where the client
  thinks the change happened and the database disagrees.
\item
...
\chapter{Citations and
References}\label{citations-and-references}}

The ideas in this book did not come from nowhere. Here are the
voices we borrowed from, and why they matter. Learning to program
is not just about code --- it is about adopting an attitude.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{``If no one asks me, I know; if I wish to explain it to
one who asks, I do not know.''} --- Augustine of Hippo,
\emph{Confessions}, Book XI (397 AD) --- Chapter 00
(Introduction)

On the gap between understanding and explaining. Reading code is
not the same as writing it. If you cannot explain what your code
does, you do not truly understand it.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{Böhm-Jacopini theorem} (1966) --- Corrado Böhm, Giuseppe
Jacopini, \emph{Flow Diagrams, Turing Machines and Languages with
Only Two Formation Rules} --- Chapter 00 (Introduction)

Any algorithm can be written using only three structures:
sequence, selection (if/else), iteration (for/while). No goto, no
tricks. This is the foundation of structured programming and a
guiding principle of this book.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{``And now for something completely different.''} ---
\emph{Monty Python's Flying Circus} (1969-1974) --- The
Apocryphal Chapter
...
\textbf{The DEC VT100} (1978) --- Chapter 00 (Introduction)

The terminal that standardized 80 columns and 24 rows. The
80-character line convention in coding standards traces back to
this machine. The terminal has been the programmer's primary
interface ever since.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{42} --- Douglas Adams, \emph{The Hitchhiker's Guide to
the Galaxy} (1979) --- Glossary, The Apocryphal Chapter
...
\textbf{``Fatti non foste a viver come bruti, ma per seguir
virtute e canoscenza.''} --- Dante Alighieri, \emph{Inferno},
Canto XXVI (c.~1320)

Ulysses speaks to his crew: ``You were not made to live as
brutes, but to follow virtue and knowledge.'' The most beautiful
speech ever written about curiosity. It is why we learn.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{Charon} (fetch) --- Dante Alighieri, \emph{Inferno},
Canto III --- How It Works

The ferryman who carries souls across the river. In our
application, \texttt{fetch} ferries data between the browser and
the server.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{Φ (Phi) --- 1.618\ldots{}} --- The golden ratio ---
Colophon
...
\textbf{``Learn to read, write and compute --- before it's too
late.''} --- The Apocryphal Chapter

The closing line of the AI section. Reading code, writing code,
and computing results are the three skills that no tool can
replace.

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\textbf{anno Domini MMXXVI / MMDCCLXXIX ab urbe condita} ---
Prelude, Colophon

The year this book was written: 2026 AD, or 2779 from the
founding of Rome. A reminder that we are part of a long history
--- and that what we build today will be read by someone in the
future.
